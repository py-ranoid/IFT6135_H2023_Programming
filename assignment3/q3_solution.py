...
class SimSiam(nn.Module):
    ...
    def loss (self, p1,p2,z1,z2, similarity_function='CosineSimilarity'):
        ...
        try:  
            # if self.stop_gradient:
            # z2_mod, z1_mod = (z2.detach(), z1.detach()) if self.stop_gradient else (z2, z1)
            loss = 0.5 * - cos(p1, z2) + 0.5 * - cos(p2, z1)
            loss = loss.mean()
        except:
            traceback.print_exc()
        return loss
...
class CosineSimilarity(Module):
    ...
    def forward(self, x1: Tensor, x2: Tensor) -> Tensor:
        ...
        """
        try:
            return torch.div(bdot(x1,x2),
                            torch.max(torch.norm(x1,dim=self.dim)*torch.norm(x2,dim=self.dim),
